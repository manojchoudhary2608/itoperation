...
const { sendDeliveryConfiguredEmail, sendFinalStatusEmail } = require('../emailService');
...
// GET all deliveries
router.get('/', (req, res) => {
    const sql = "SELECT * FROM deliveries ORDER BY delivery_date DESC";
    db.all(sql, [], (err, rows) => {
        if (err) {
...
router.post('/', (req, res) => {
    ...
    db.run(sql, params, function (err) {
        ...
        if (it_status === 'Configured') {
            sendDeliveryConfiguredEmail({ id: this.lastID, ...req.body });
        }
    });
});
...
router.put('/:id', (req, res) => {
    ...
    // First, get the original final_status and new_joiner
    db.get("SELECT final_status, new_joiner FROM deliveries WHERE id = ?", [id], (err, originalDelivery) => {
        if (err) {
        ...
        const originalFinalStatus = originalDelivery.final_status;
        ...
        const params = [name, address, asset_type, mobile_number, courier_partner, tracking_number, courier_date, it_status, final_status, delivery_date, new_joiner, id];
        ...
        db.run(sql, params, function (err) {
            ...
            if (this.changes > 0) {
                // Check for IT Status configured email
                if (it_status === 'Configured') {
                    sendDeliveryConfiguredEmail({ id: id, ...req.body });
                }

                // Check for Final Status change email
                if (final_status !== originalFinalStatus) {
                    let toRecipients = [];
                    let ccRecipients = [];

                    if (new_joiner === 'Yes') {
                        toRecipients = ["ithelpdesk@exelonglobal.com", "careers.exelon@exelonglobal.com"];
                        ccRecipients = ["prathap.srinivasa@exelonglobal.com", "putta.chennaiah@exelonglobal.com"];
                    } else if (new_joiner === 'No') {
                        toRecipients = ["ithelpdesk@exelonglobal.com"];
                        ccRecipients = ["putta.chennaiah@exelonglobal.com"];
                    }

                    if (toRecipients.length > 0) {
                        sendFinalStatusEmail({ id: id, ...req.body }, toRecipients, ccRecipients);
                    }
                }
            }
            ...
            res.json({
                message: "success",
                data: { id: id, changes: this.changes, ...req.body }
            });
        });
    });
});
                    ...
                    function insertDelivery(index) {
                        ...
                        stmt.run([delivery.name, delivery.address, delivery.asset_type, delivery.mobile_number, delivery.courier_partner, delivery.tracking_number, delivery.courier_date, delivery.it_status, delivery.final_status, delivery.delivery_date, delivery.new_joiner], function (err) {
                            if (err) {
                                ...
                                if (delivery.it_status === 'Configured') {
                                    sendDeliveryConfiguredEmail({ id: this.lastID, ...delivery });
                                }
                            }
