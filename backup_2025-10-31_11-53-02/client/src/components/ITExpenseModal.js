...
const ITExpenseModal = ({ isOpen, onClose, onSubmit, initialData, formData, setFormData }) => {
    ...
    const calculateTotalAmount = () => {
        return formData.items.reduce((sum, item) => sum + (item.item_amount || 0), 0);
    };
                ...
                <form onSubmit={handleSubmit}>
                    <div className="row">
                        <div className="col-md-6 mb-3">
                            ...
                            <input type="text" className="form-control" id="vendor_name" name="vendor_name" value={formData.vendor_name} onChange={handleInputChange} required />
                        </div>
                        <div className="col-md-6 mb-3">
                            <label htmlFor="invoice_number" className="form-label">Invoice Number</label>
                            <input type="text" className="form-control" id="invoice_number" name="invoice_number" value={formData.invoice_number} onChange={handleInputChange} required />
                        </div>
                    </div>
                    <div className="row">
                        <div className="col-md-6 mb-3">
                            <label htmlFor="invoice_date" className="form-label">Invoice Date</label>
                            <input type="date" className="form-control" id="invoice_date" name="invoice_date" value={formData.invoice_date} onChange={handleInputChange} required />
                        </div>
                        <div className="col-md-6 mb-3">
                    ...
                    {formData.items.map((item, index) => (
                        <div key={index} className="row mb-2 align-items-end border p-2 rounded">
                            ...
                            <div className="col-md-2 mb-2">
                                ...
                                <input type="number" step="0.01" className="form-control" id={`tax_percentage_${index}`} name="tax_percentage" value={item.tax_percentage} onChange={(e) => handleItemChange(index, e)} />
                            </div>
                            <div className="col-md-2 mb-2">
