...
router.post('/', (req, res) => {
    ...
    const insertInvoiceSql = 'INSERT INTO invoices (vendor_name, invoice_number, invoice_date, total_amount, invoice_file_path) VALUES (?,?,?,?,?)';
    const insertInvoiceParams = [vendor_name, invoice_number, invoice_date, total_amount, invoice_file_path];
            ...
            db.run(insertItemSql, insertItemParams, (err) => {
                ...
                if (itemsInserted === items.length) {
                    db.run("COMMIT;", (commitErr) => {
                        ...
                        res.json({
                            "message": "success",
                            "data": { id: invoice_id, ...req.body, total_amount, invoice_file_path }
                        });
                    });
                }
            });
...
router.put('/:id', (req, res) => {
    ...
    const updateInvoiceSql = 'UPDATE invoices SET vendor_name = ?, invoice_number = ?, invoice_date = ?, total_amount = ?, invoice_file_path = COALESCE(?, invoice_file_path) WHERE id = ?';
    const updateInvoiceParams = [vendor_name, invoice_number, invoice_date, total_amount, invoice_file_path, invoice_id];
