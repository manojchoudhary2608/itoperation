...
router.post('/', (req, res) => {
    ...
    db.run(sql, params, function (err) {
        ...
        const user = req.headers['x-user'] || 'system'; // Assuming user is sent in headers

        const historySql = 'INSERT INTO asset_history (asset_id, user, action, new_value) VALUES (?, ?, ?, ?)';
        const historyParams = [newAssetId, user, 'create', JSON.stringify(req.body)];
        db.run(historySql, historyParams);

        res.json({
            "message": "success",
            "data": { id: newAssetId, ...req.body }
        });
    });
});
...
router.put('/:id', (req, res) => {
    ...
    db.get("SELECT * FROM assets WHERE id = ?", [assetId], (err, oldAsset) => {
        if (err) {
            res.status(400).json({"error":err.message});
            return;
        }
        ...
        db.run(sql, params, function (err) {
            ...
            const newAsset = { id: assetId, ...req.body };
            const changes = [];
            for (const key in newAsset) {
                if (key !== 'id' && oldAsset[key] !== newAsset[key]) {
                    changes.push({
                        field: key,
            ...
            if (changes.length > 0) {
                const historySql = 'INSERT INTO asset_history (asset_id, user, action, field, old_value, new_value) VALUES (?, ?, ?, ?, ?, ?)';
                changes.forEach(change => {
                    db.run(historySql, [assetId, user, 'update', change.field, change.old_value, change.new_value]);
                });
            }

            res.json({
                message: "success",
                data: { id: assetId, changes: this.changes, ...req.body }
            });
        });
    });
});
...
router.delete('/:id', (req, res) => {
    ...
    db.run(sql, params, function (err) {
        ...
        const historySql = 'INSERT INTO asset_history (asset_id, user, action) VALUES (?, ?, ?)';
        db.run(historySql, [assetId, user, 'delete']);
...
router.get('/:id/history', (req, res) => {
    const sql = "SELECT * FROM asset_history WHERE asset_id = ? ORDER BY timestamp DESC";
    const params = [req.params.id];
...
router.post('/upgrade-primary', (req, res) => {
    const { oldAssetId, oldAssetNewStatus, newAssetId } = req.body;

    if (!oldAssetId || !oldAssetNewStatus || !newAssetId) {
    ...
    db.serialize(() => {
        ...
        db.get("SELECT * FROM assets WHERE id = ?", [oldAssetId], (err, oldAsset) => {
            if (err || !oldAsset) {
                db.run("ROLLBACK;");
                return res.status(404).json({ error: "Old asset not found." });
            }

            const employeeDetails = {
                assigned_to: oldAsset.assigned_to,
                gaid: oldAsset.gaid,
                email_id: oldAsset.email_id,
                reporting_manager: oldAsset.reporting_manager,
                manager_email_id: oldAsset.manager_email_id
            };

            const peripheralDetails = {
                monitor1_asset_tag: oldAsset.monitor1_asset_tag,
                monitor1_serial_number: oldAsset.monitor1_serial_number,
                monitor2_asset_tag: oldAsset.monitor2_asset_tag,
                monitor2_serial_number: oldAsset.monitor2_serial_number,
                headset_asset_tag: oldAsset.headset_asset_tag,
                headset_serial_number: oldAsset.headset_serial_number,
                yubikey_number: oldAsset.yubikey_number,
                webcam_number: oldAsset.webcam_number
            };

            const updateOldSql = `UPDATE assets SET
                                status = ?, assigned_to = '', gaid = '', email_id = '',
                                monitor1_asset_tag = '', monitor1_serial_number = '',
                                monitor2_asset_tag = '', monitor2_serial_number = '',
                                headset_asset_tag = '', headset_serial_number = '',
                                yubikey_number = '', webcam_number = '',
                                reporting_manager = '', manager_email_id = ''
                              WHERE id = ?`;
            db.run(updateOldSql, [oldAssetNewStatus, oldAssetId], function(err) {
                if (err) {
                    db.run("ROLLBACK;");
                    return res.status(500).json({ error: `Failed to update old asset: ${err.message}` });
                }

                const updateNewSql = `UPDATE assets SET
                                    status = 'Assigned',
                                    assigned_to = ?, gaid = ?, email_id = ?,
                                    reporting_manager = ?, manager_email_id = ?,
                                    monitor1_asset_tag = ?, monitor1_serial_number = ?,
                                    monitor2_asset_tag = ?, monitor2_serial_number = ?,
                                    headset_asset_tag = ?, headset_serial_number = ?,
                                  ...
                                  WHERE id = ?`;
                const params = [
                    employeeDetails.assigned_to, employeeDetails.gaid, employeeDetails.email_id,
                    employeeDetails.reporting_manager, employeeDetails.manager_email_id,
                    peripheralDetails.monitor1_asset_tag, peripheralDetails.monitor1_serial_number,
                    peripheralDetails.monitor2_asset_tag, peripheralDetails.monitor2_serial_number,
                    peripheralDetails.headset_asset_tag, peripheralDetails.headset_serial_number,
                    peripheralDetails.yubikey_number, peripheralDetails.webcam_number,
                    newAssetId
                ];
                db.run(updateNewSql, params, function(err) {
                    if (err) {
                        db.run("ROLLBACK;");
                        return res.status(500).json({ error: `Failed to assign new asset: ${err.message}` });
                    }
                    ...
                    db.run("COMMIT;", (commitErr) => {
                        ...
                        res.json({ message: "Asset replaced successfully." });
                    });
                });
            });
        });
...
router.post('/swap-peripheral', (req, res) => {
    const { primaryAssetId, field, newPeripheralId } = req.body;

    if (!primaryAssetId || !field || !newPeripheralId) {
        return res.status(400).json({ error: "Missing required fields for peripheral swap." });
    }

    db.serialize(() => {
        db.run("BEGIN TRANSACTION;");

        db.get("SELECT * FROM assets WHERE id = ?", [primaryAssetId], (err, primaryAsset) => {
            if (err || !primaryAsset) {
                db.run("ROLLBACK;");
                return res.status(404).json({ error: "Primary asset not found." });
            }

            db.get("SELECT * FROM assets WHERE id = ?", [newPeripheralId], (err, newPeripheral) => {
                if (err || !newPeripheral) {
                    db.run("ROLLBACK;");
                    return res.status(404).json({ error: "New peripheral asset not found." });
                }

                const oldTag = primaryAsset[`${field}_asset_tag`];
                const oldSN = primaryAsset[`${field}_serial_number`];
                const oldType = field.includes('monitor') ? 'Monitor' : 'Headset';

                const createOldAssetAsStock = (callback) => {
                    if (!oldTag) return callback(); // No old peripheral to put in stock
                    const sql = 'INSERT INTO assets (asset_tag, serial_number, asset_type, status) VALUES (?, ?, ?, ?)';
                    db.run(sql, [oldTag, oldSN, oldType, 'In Stock'], callback);
                };

                createOldAssetAsStock(err => {
                    if (err) {
                        db.run("ROLLBACK;");
                        return res.status(500).json({ error: `Failed to create stock asset for old peripheral: ${err.message}` });
                    }

                    const updatePrimarySql = `UPDATE assets SET 
                                                ${field}_asset_tag = ?,
                                                ${field}_serial_number = ?
                                            WHERE id = ?`;
                    db.run(updatePrimarySql, [newPeripheral.asset_tag, newPeripheral.serial_number, primaryAssetId], function(err) {
                        if (err) {
                            db.run("ROLLBACK;");
                            return res.status(500).json({ error: `Failed to update primary asset: ${err.message}` });
                        }

                        const updateNewPeripheralSql = "UPDATE assets SET status = 'Assigned' WHERE id = ?";
                        db.run(updateNewPeripheralSql, [newPeripheralId], function(err) {
                            if (err) {
                                db.run("ROLLBACK;");
                                return res.status(500).json({ error: `Failed to assign new peripheral: ${err.message}` });
                            }
                            ...
                            db.run("COMMIT;", (commitErr) => {
                                ...
                                res.json({ message: "Peripheral replaced successfully." });
                            });
                        });
                    });
                });
