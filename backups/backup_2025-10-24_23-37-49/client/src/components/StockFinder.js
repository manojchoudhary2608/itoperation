...
const StockFinder = () => {
    ...
    const onAddItem = async (itemData) => {
        try {
            const existingItem = stockItems.find(item => item.item_name === itemData.item_name);

            if (existingItem) {
                // Item exists, so update it
                const updatedPurchaseQty = existingItem.purchase_qty + itemData.purchase_qty;
                await axios.put(`${API_URL}/${existingItem.id}`, {
                    ...existingItem,
            ...
            fetchStockItems(); // Refresh the list in both cases
        } catch (err) {
            // Re-throw to allow modal to catch and display error
            throw err;
        }
    };
    ...
    const handleExportCsv = () => {
        ...
        const headers = ["ID", "Item Name", "Purchase Qty", "Assign Qty", "Stock Balance"];
        const csvRows = [];
    ...
    return (
        <div className="container mt-4" style={{ backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '8px' }}>
            ...
            <div className="d-flex justify-content-end mb-3">
                ...
                <button className="btn btn-info me-2" onClick={() => setIsAssignItemModalOpen(true)}>
                    Assign Item
                </button>
