...
const { sendNewHireEmail } = require('../emailService');
...
router.post('/', (req, res) => {
    ...
    db.run(sql, params, function (err) {
        ...
        sendNewHireEmail({ id: this.lastID, name, address, mobile_number, date_of_joining, status });
    });
});
...
router.put('/:id', (req, res) => {
    ...
    const params = [name, address, mobile_number, date_of_joining, status, req.params.id];
            db.run(sql, params, function (err) {
                ...
                if (this.changes > 0) {
                    db.get("SELECT * FROM new_hires WHERE id = ?", [req.params.id], (err, row) => {
                        if (err) {
                            console.error("Error fetching updated new hire for email:", err.message);
                        } else if (row) {
                            sendNewHireEmail(row);
                        }
                    });
                }
                res.json({
