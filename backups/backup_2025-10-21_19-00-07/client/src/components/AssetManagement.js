...
const AssetManagement = () => {
    ...
    const [selectedAssetId, setSelectedAssetId] = useState(null);
        ...
        const handleInputChange = (e) => {
            ...
            const handleShowHistoryModal = async (assetId) => {
                setSelectedAssetId(assetId);
                try {
                    const response = await axios.get(`${API_URL}/${assetId}/history`);
    ...
    return (
        <Container fluid className="mt-4" style={{ backgroundColor: '#f8f9fa', padding: '20px', borderRadius: '8px' }}>
                    ...
                    <tbody>
                        {filteredAssets.length > 0 ? filteredAssets.map(asset => (
                            <tr key={asset.id}>
                                ...
                                <td>{asset.status}</td>
                                <td>
                                    <Button variant="info" size="sm" onClick={() => handleShowHistoryModal(asset.id)} className="me-2">History</Button>
            ...
            <Modal show={showModal} onHide={handleCloseModal} size="lg">
                ...
                <Modal.Body style={{ backgroundColor: '#e3f2fd' }}>
