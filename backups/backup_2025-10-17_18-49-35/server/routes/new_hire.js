...
router.put('/:id', (req, res) => {
    ...
    db.get("SELECT status, created_at FROM new_hires WHERE id = ?", [id], (err, originalHire) => {
        ...
        db.run(sql, params, function (err) {
            ...
            if (changes > 0 && newStatus === 'Close' && originalStatus !== 'Close') {
            ...
            } else if (changes > 0 && newStatus === 'Called Off' && originalStatus !== 'Called Off') {
                console.log("Status changed to Called Off, sending email...");
                db.get("SELECT * FROM new_hires WHERE id = ?", [id], (err, updatedHire) => {
                    if (err) {
                        console.error("Error fetching updated new hire for email:", err.message);
                    } else if (updatedHire) {
                        sendNewHireCalledOffEmail(updatedHire);
                    }
                    return res.json({ message: "success", changes: changes });
                });
            } else if (changes > 0) {
